...
from: "ERC-20"
to: "LSP7"
summary: "A step-by-step migration from an ERC-20 token contract to LSP7. Map balances, transfers, approvals, metadata, and receiver behavior."
quotableAnswer: >-
  Migrating an ERC-20 token to LSP7 maps `transfer(to, amount)` to `transfer(from, to, amount, force, data)`; `approve` and `allowance` become `authorizeOperator(operator, amount, data)`. The receiver-hook surface changes from silent to LSP1 `universalReceiver(typeId, data)` fired on every transfer. Metadata moves from `name()`/`symbol()`/`decimals()` to LSP4 typed keys under ERC-725Y. The balance-map schema is preserved. The migration takes hours for a fresh deployment and days for a live token with existing allowances to snapshot and re-authorize.
